...
class SocketHandler {
  ...
  handleUserEvents(socket) {
    ...
    socket.on('update_status', async (data) => {
      try {
        ...
        this.notifyContactsStatusChange(socket.userId, status);
  ...
  handleDisconnect(socket) {
    ...
    // Notify contacts about status change
    this.notifyContactsStatusChange(socket.userId, 'offline');
  ...
  async notifyContactsStatusChange(userId, status) {
    try {
      ...
      for (const chat of chats) {
        for (const participant of chat.participants) {
          if (participant.user._id.toString() !== userId && participant.isActive) {
            const contactRoom = `user_${participant.user._id}`;
            this.io.to(contactRoom).emit('contact_status_change', {
              userId,
              status,
              timestamp: new Date()
            });
      ...
      console.error('Notify contacts status change error:', error);
    }
  }
