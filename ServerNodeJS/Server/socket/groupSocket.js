...
class GroupSocketHandler {
  ...
  handleConnection(socket) {
    ...
    socket.on('join_group', async (data) => {
      try {
        ...
        await this.updateUserGroupStatus(userId, groupId, 'online');
        ...
        const onlineMembers = await this.getOnlineGroupMembers(groupId);
    ...
    socket.on('leave_group', async (data) => {
      try {
        ...
        // Update user status in group
        await this.updateUserGroupStatus(userId, groupId, 'offline');
    ...
    // Handle group member status update
    socket.on('update_group_status', async (data) => {
      try {
        ...
        // Update user status in group
        await this.updateUserGroupStatus(userId, groupId, status);

        // Broadcast status update to group room
        const roomName = `group_${groupId}`;
        this.io.to(roomName).emit('group_member_status_update', {
          groupId,
          userId,
          status,
          timestamp: new Date()
  ...
  // Helper methods
  async updateUserGroupStatus(userId, groupId, status) {
    try {
      // Update user's general status
      ...
      console.error('Update user group status error:', error);
    }
  }

  async getOnlineGroupMembers(groupId) {
    try {
      const group = await Group.findById(groupId)
      ...
      return group.members
        .filter(member => member.isActive && member.user.status === 'online')
        .map(member => ({
      ...
      console.error('Get online group members error:', error);
      return [];
    }
  }
