...
const initiateCall = async (req, res) => {
    try {
        ...
        if (existingCall) {
            // Force end without full-doc validation (legacy calls may lack roomData.roomId)
            const endPatch = {
                status: 'ended',
                endedAt: new Date(),
                duration: Math.floor((new Date() - existingCall.startedAt) / 1000),
                'roomData.roomEnded': true
            };
            if (!existingCall.roomData?.roomId) {
                endPatch['roomData.roomId'] = generateRoomId();
            }
            await Call.updateOne({ _id: existingCall._id }, { $set: endPatch }, { runValidators: false });
