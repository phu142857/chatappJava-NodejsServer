...
const getServerHealth = async (req, res) => {
  try {
    // Get CPU usage directly from system using systeminformation
    // This gets the current CPU usage percentage from the VPS/system
    const cpuData = await si.currentLoad();
    const cpuPercent = Math.round(cpuData.currentload || 0);
